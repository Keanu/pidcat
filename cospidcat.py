...
PID_START = re.compile(r'KPI_PATTERN ([a-zA-Z0-9._:]+)\.([a-zA-Z0-9]+)@[a-zA-Z0-9]+ onCreate, [0-9]ms$')
PID_KILL  = re.compile(r'^Killing (\d+):([a-zA-Z0-9._]+)/[^:]+: (.*)$')
PID_LEAVE = re.compile(r'^No longer want ([a-zA-Z0-9._]+) \(pid (\d+)\): .*$')
PID_DEATH = re.compile(r'KPI_PATTERN ([a-zA-Z0-9._:]+)\.([a-zA-Z0-9]+)\.([a-zA-Z0-9]+)@[a-zA-Z0-9]+ onDestroy, [0-9]ms$')
...
while adb.poll() is None:
  ...
  if start is not None:
    line_package, target = start.groups()
    ...
    if match_packages(line_package):
      ...
      linebuf += indent_wrap(' Process created for %s.%s\n' % (line_package, target))
      linebuf += colorize(' ' * (header_size - 1), bg=WHITE)
      linebuf += ' PID: %s' % owner
      linebuf += '\n'
      print(linebuf)
